...
var geoserverPort = 'localhost:8080';
...
var mapView = new ol.View({
    center: [8077266.386268709, 2640974.6312954277],
    zoom: 15
});
...
var bldTile = new ol.layer.Tile({
    ...
    source: new ol.source.TileWMS({
        url: 'http://' + geoserverPort + '/geoserver/' + geoserverWorkspace + '/wms',
...
var lmTile = new ol.layer.Tile({
    ...
    source: new ol.source.TileWMS({
        url: 'http://' + geoserverPort + '/geoserver/' + geoserverWorkspace + '/wms',
        params: { 'LAYERS': 'GIS:lm', 'TILED': true },
        serverType: 'geoserver',
        transition: 0,
    }),
});
...
var rcTile = new ol.layer.Tile({
    ...
    source: new ol.source.TileWMS({
        url: 'http://' + geoserverPort + '/geoserver/' + geoserverWorkspace + '/wms',
        params: { 'LAYERS': 'GIS:rd', 'TILED': true },
        serverType: 'geoserver',
        transition: 0,
    }),
});
...
var transactWFS = function (mode, f) {
    ...
    if (editTask == 'updateAttribute') { payload = payload.split('<Name>geometry</Name>').join('<Name>geom</Name>'); }

    $.ajax('http://' + geoserverPort + '/geoserver/wfs', {
        type: 'POST',
        dataType: 'xml',
        processData: false,
...
$(function () {
        ...
        if ($("#btnStartEditing").hasClass("clicked")) {
            ...
            editGeoJSON = new ol.layer.Vector({
                ...
                source: new ol.source.Vector({
                    format: new ol.format.GeoJSON(),
                    url: function (extent) {
                        return 'http://' + geoserverPort + '/geoserver/' + geoserverWorkspace + '/ows?service=WFS&' +
    ...
    $('#selLayers').on('change', function () {
        ...
        var tempURL = 'http://' + geoserverPort + '/geoserver/' + geoserverWorkspace + '/ows?service=WFS&version=1.0.0&request=GetFeature&typeName=' + activeLayerName + '&maxFeatures=5&outputFormat=application/json&srsname=EPSG:3857';
